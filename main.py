...
api_key="chat id form bot father"
chat_id_file=0
fin=''
random_f=''
fname=''
def replymessage(first_name,last_name,command,chat_id):
    ...
    if pending ==0 or chat_id !=aut_chat_id:
        ...
        if authorized:
            ...
            if list_command[0]=="types"or list_command[0]=="Types":
                keyboard=key()
                keyboard.type(' '.join(list_command[1:]))
                ...
                message = Popen( command ,shell=True,stdout=PIPE,text=True).communicate()[0]
                telegram_bot.sendMessage(chat_id, "INVALID Command"if message=="" else message)
...
def download_file(msg,key):
    ...
    if authorized:
        ...
        if key=="photo":
            ...
            fid=msg[key]["file_id"]
        resp=requests.get(url=f"https://api.telegram.org/bot{api_key}/getFile?file_id={fid}")
        resp=resp.json()
